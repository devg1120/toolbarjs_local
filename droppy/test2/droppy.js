...
document.addEventListener('click', (event) => {
    for (const instance of clickAwayContext.instances) {
        if (instance.drop.checkVisibility()
            ...
            && ! instance.drop.contains(event.target)) {
		    instance.toggle();
	}  
    }
 ...
 */
export function menuGenerator(root, options, trigger_handler = null, context = new DroppyContext()) {
    ...
    for (const wrapper of wrappers) {
        ...
        if (! trigger || ! drop) {
           if (trigger) {
                ...
                //console.log("trigger click", trigger.href);
               //});
		if (trigger_handler != null) {
                    trigger.addEventListener('click', trigger_handler);
		}
	   }
           continue;
	}
